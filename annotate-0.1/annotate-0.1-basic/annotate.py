...
#			print list(genome[chromosome_conversion(l[0])])[snp_pos], l[3]	#this line will print the reference base from the BED file and genome, to check for concordance.
			...
			for g in genes:
				...
				if genes[g][6] == chrom_conv:
					...
					if genes[g][0].split('-')[0][-1] == 'C':
						...
						if snp_pos <= genes[g][1] and snp_pos >= genes[g][2]:
							...
							for intron in genes[g][4]:
								if snp_pos <= intron[0] and snp_pos >= intron[1]:
									...
									if snp_pos in xrange(intron[0]-2, intron[0]+1) or snp_pos in xrange(intron[1],intron[1]+3):
						...
						elif snp_pos in xrange(genes[g][1]+1,genes[g][1]+upstream+1):
							print >> f_out, '\t'.join(l + ["5'-upstream", genes[g][0], str(genes[g][1]-snp_pos)])
							annotation = True	
					...
					#WATSON STRAND
						...
						if snp_pos >= genes[g][1] and snp_pos <= genes[g][2]:
							...
							for intron in genes[g][4]:
								if snp_pos >= intron[0] and snp_pos <= intron[1]:
									...
									if snp_pos in xrange(intron[0],intron[0]+3) or snp_pos in xrange(intron[1]-2,intron[1]+1):
										print >> f_out, '\t'.join(l + ['splice-site', genes[g][0], 'NA'])
						...
						elif snp_pos in xrange(genes[g][1]-upstream,genes[g][1]):
							print >> f_out, '\t'.join(l + ["5'-upstream", genes[g][0], str(snp_pos-genes[g][1])])
							annotation = True						
