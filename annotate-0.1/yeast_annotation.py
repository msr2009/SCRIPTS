...
def main(BED, orfs, noncoding_file, genome_file, upstream, exclude):
	...
	print genes['YGL226C-A']
	(genes, noncoding) = parseGFF(noncoding_file, exclude)	
	print genes['YGL226C-A']
...
def parseGFF(gff, exclude):
	...
	for line in open(gff, 'r'):
		...
		if l[2] == "gene":
			genes[ann_list['ID']] = [ann_list['ID'], 0, 0, [  ], [  ], 0, chromosome_conversion(l[0][3:]), l[6] ]		
	...
	#before returning, we have to add the start and stop positions () and flip the positions for any gene that's on the Crick strand
	for g in genes:
		#get min and max positions in exons
		cds_min = int(min([min(x) for x in genes[g][3]]))
		cds_max = int(max([max(x) for x in genes[g][3]]))
		
		if genes[g][-1] == "-":
			genes[g] = [ genes[g][0], cds_max, cds_min, [ x[::-1] for x in genes[g][3] ][::-1], [ x[::-1] for x in genes[g][4] ][::-1], (sum([ abs(x-y) for x,y in genes[g][3] ])+1)/3.0, genes[g][6], genes[g][7] ]
		else:
...
if __name__ == "__main__":
	...
	main(option.inputfile, option.sequences, option.noncoding, option.genome, option.upstream, exclude)
