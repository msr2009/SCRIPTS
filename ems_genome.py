...
def gene_mutation(name, mut_pos, orf_file):
	return
...
def mutate_genome(reference_genome, expected):
	...
	mutate = (expected+1)/((total_length+1)*(gc/total_length))
	...
	for chromosome in genome:
		...
		for position in range(len(chromosome)):
			...
			elif trans >= mutate*0.01 and trans < mutate:
				if chromosome[position] == 'G':
					mutated_chromosome += 'A'
				elif chromosome[position] == 'C':
					mutated_chromosome += 'T'
				else:
					mutated_chromosome += chromosome[position]
			elif trans >= mutate:
				mutated_chromosome += chromosome[position]
