...
def update_qc_file(qcfile, new_values):
	'''define a function that updates the JSON-encoded QC file with new data item(s) (in dictionary)'''
	
	import json
	
	#read this way to allow for pretty printing
	qc_dict = json.loads( ''.join( map(lambda x: x.strip(), open(qcfile,'r').readlines()) ) )
	qc_dict.update(new_values)
	print >> open(qcfile, 'w'), json.dumps(qc_dict)
