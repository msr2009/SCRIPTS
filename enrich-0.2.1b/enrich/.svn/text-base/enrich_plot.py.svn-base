#!/usr/bin/env python
'''
enrich_plot: contains a number of functions that use matplotlib to generate visualizations of enrich data.	This module is inteded to be extensible, containing all enrich-related visuals
'''
...
def ratioplot(path, infile1, infile2):
	...
	if log_value == True:
		ax = plt.plot( [positions[index] for index in xrange(len(ratios)) if ratios[index] > 0],
				 [ratios[index] for index in xrange(len(ratios)) if ratios[index] > 0], 'ro')
		ax = plt.plot( [positions[index] for index in xrange(len(ratios)) if ratios[index] <= 0],
				 [ratios[index] for index in xrange(len(ratios)) if ratios[index] <= 0], 'bo')
	else:
	...
	if log_value == True:
		plt.vlines(positions, [0] * len(positions), ratios, color=colors, linestyles='solid', lw=2)
...
def separate_data(substitution, positions, ratio, wt_data, axis_num, cols, rows, offset):
	'''
	...
	header_tick_values = range(min(positions), min(positions)+len(values), (len(values)/4))

	all_residue_plot_data(wt_x, wt_y, nan_x, nan_y, rest_x, rest_y) 
	adjust_axes(ax, str(substitution), header_tick_values, header_tick_values, axis_num, cols, 18, offset)
	
def AAPlot(wt_data, ratio, axis_num, positions, offset):
	...
	for substitution in keys:
		if (substitution != '*'): # Save the stop index for last
			axis_num += 1
			# a : {0, 1, 2, 0, ... }
			separate_data(substitution, positions, ratio, wt_data, axis_num, cols, rows, offset)
...
def PosPlot(wt_data, ratio, axis_num, positions, offset):
	...
	for substitution in aa_sorted:
		if (substitution != '*'):
			...
			for index in xrange(len(ratio[substitution])):
				if position_d.get(index+offset,None) == None:
					position_d[index+offset] = []
				position_d[index+offset].append(ratio[substitution][index])

	position_d['header'].append('*')
	for index in xrange(len(ratio['*'])):
		if position_d.get(index+offset, None) == None:
			position_d[index+offset] = []
		position_d[index+offset].append(ratio['*'][index])
	...
	for position in keys:
		...
		'''
		wt_x = [positions[index] for index in xr if (values[index] == 0 and wt_data['variable'][position-offset] == position_d['header'][index])]
		wt_y = [values[index] for index in xr if (values[index] == 0 and wt_data['variable'][position-offset] == position_d['header'][index])]
		nan_x = [positions[index] for index in xr if (values[index] == 0 and wt_data['variable'][position-offset] != position_d['header'][index])]
		nan_y = [values[index] for index in xr if (values[index] == 0 and wt_data['variable'][position-offset] != position_d['header'][index])]
		rest_x = [positions[index] for index in xr if values[index] != 0]
		rest_y = [values[index] for index in xr if values[index] != 0]
		...
		all_residue_plot_data(wt_x, wt_y, nan_x, nan_y, rest_x, rest_y)
		adjust_axes(ax, str(position), range(offset, len(position_d['header'])+offset), position_d['header'], axis_num, cols, 13, offset)

def adjust_axes(ax, title, header_ticks, header_list, axis_num, cols, axesfontsize, offset):
	'''
	...
	plt.text(offset+.1,3, title, **title_font)
...
def all_residue_plot(path, infile1, infile2, wtseq, mode, offset):
	'''
	...
	try:
		...
		print 'Error: could not cast offset at integer'
		return 1
	...
	'''
	Read in the data
	file1_values/file2_values: a dictionary mapping {'position': 'frequency'}
	...
	ylab = ""
	if mode is 'AA':
		AAPlot(wt_data, ratio, axis_num, positions, offset)
		ylab = "Position"
	elif mode is 'Pos':
		PosPlot(wt_data, ratio, axis_num, positions, offset)
