...
__author__ = "Ikjin Kim"
__copyright__ = "Copyright 2012"
__credits__ = ["Ikjin Kim", "Matt S Rich"]
__license__ = "FreeBSD"
__version__ = "0.2"
...
def tile_info(path, file_name, error_code_readID_list):
	try:
		...
		for summary_name in summary_list:
			...
			print >> f_output, '\t'.join(['lane_number', 'tile_name', 'X_coordinate', 'Y_coordinate'])

			for read_ID in error_code_readID_list[i]:
				read_ID_list = read_ID.split(':')
				print >> f_output, '\t'.join(read_ID_list[1:5])
			i += 1
