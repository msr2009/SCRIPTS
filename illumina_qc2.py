...
def main(filename):	
	...
	while lenbit:
		...
		for i in range(0, ((len(read1_lines)/4)-lenbit)):
			...
			for s in range(len(read1_quality)):
				if read1_quality[s] in base_quality[s]:
					base_quality[s][read1_quality[s]] += 1
				else:
					base_quality[s][read1_quality[s]] = 1 
	...
	qc_dict = {'average_quality': base_quality, 'tile_quality': tile_info}
...
#	calcAverage(qc_dic['average_quality'])
	
	scores = []
	for i in qc_dic['average_quality']:
		scores.append( expandHist(i) )
	...
	plt.xlabel('Tile')
	plt.xticks(pos+width/2.0, sorted(tiles), rotation=90)
	plt.ylabel('Frequency of reads passing chastity filter')
	plt.title('Read chastity by tile: ' + prefix.split('/')[-1])
	plt.savefig(prefix+'_tile-chastity.pdf',  bbox_inches='tight')


def expandHist(histdic):
	all_scores = []
	for score in histdic:
		for i in range(histdic[score]):
			all_scores.append(int(score))
	return all_scores
	
	
def calcAverage(quality_hist):
	means = []
	for pos in quality_hist:
		total_reads = 0
		total_qual = 0
		for score in pos:	
			total_reads += int(pos[score])
			total_qual += int(score)*int(pos[score])
		means.append(total_qual/total_reads)		
	
	stds = []
	for pos in quality_hist:
		all_scores = []
		for score in pos:
			for i in range(pos[score]):
				all_scores.append(int(score))
		stds.append(std(all_scores))
